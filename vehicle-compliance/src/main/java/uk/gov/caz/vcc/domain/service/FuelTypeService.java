...
public class FuelTypeService {
   ...
   * @throws NotFoundException If the fuel type cannot be converted throw
   *                           NotFound.
   */
  public String getFuelType(String type) {
    if (Strings.isNullOrEmpty(type)) {
      throw new UnrecognisedFuelTypeException(
          String.format("%s%s", UNRECOGNISED_FUEL, "null"));
    }
    if (petrolTypes.contains(type.toLowerCase())) {
      ...
      throw new UnrecognisedFuelTypeException(
          String.format("%s%s", UNRECOGNISED_FUEL, type));
    }
  }

  public FuelTypeService(
    @Value("${application.leeds.exempt-fuel-types:steam,electricity,fuel cells,gas}") String[] exemptFuelTypes,
    @Value("${application.petrol-types:petrol,hybrid electric,gas bi-fuel,gas/petrol}") String[] petrolTypes,
    @Value("${application.diesel-types:diesel,heavy oil,electric diesel,gas diesel}") String[] dieselTypes) {
  ...
  public Set<String> getExemptFuelTypes() {
