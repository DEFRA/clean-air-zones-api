...
import uk.gov.caz.vcc.dto.validation.VehicleEntrantTimestampValidator.TimestampNotNullValidator;
import uk.gov.caz.vcc.dto.validation.VehicleEntrantTimestampValidator.TimestampValidFormatValidator;

class VehicleEntrantTimestampValidatorTest {

  static Stream<TestCase> testCases() {
    return Stream.of(
        TestCase
            .withTimestamp(null).withVrn("PO4CK71")
            .shouldReturnError(true).forValidator(new TimestampNotNullValidator()),
        TestCase
            .withTimestamp("").withVrn("PO4CK71")
            .shouldReturnError(true).forValidator(new TimestampValidFormatValidator()),
        TestCase.withTimestamp(" ").withVrn("PO4CK71").shouldReturnError(true)
            .forValidator(new TimestampValidFormatValidator()),
        TestCase.withTimestamp("2017-10-01T155301Z").withVrn("PO4CK71")
            .shouldReturnError(false).forValidator(new TimestampValidFormatValidator())
    );
  }

  @ParameterizedTest
  @MethodSource("testCases")
  void shouldTestTimestampValidator(TestCase testCase) {
    VehicleEntrantDto vehicleEntrantDto = new VehicleEntrantDto(testCase.vrn, testCase.timestamp);

    assertThat(VehicleEntrantTimestampValidator.INSTANCE.validate(vehicleEntrantDto)).matches(
        shouldPass(testCase), "Tested timestamp: " + testCase.timestamp
    );
  }
