package uk.gov.caz.testlambda.amazonaws;
...
import uk.gov.caz.testlambda.Application;
...
public class StreamLambdaHandler implements RequestStreamHandler {
  ...
  public void handleRequest(InputStream inputStream, OutputStream outputStream, Context context)
    ...
    if (log.isDebugEnabled()) {
      log.debug("Incoming attributes: " + dump(new ByteArrayInputStream(input.getBytes())));
      log.debug("Incoming context: " + dump(context));
    }
    initializeHandlerIfNull();
    log.info("Lambda start processing request at {}ms", System.currentTimeMillis());
