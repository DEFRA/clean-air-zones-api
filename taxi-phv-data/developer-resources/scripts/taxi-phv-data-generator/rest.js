...
for(let i = 0; i < recordsCnt; i++) {
  ...
  const appendComma = i < recordsCnt - 1 ? "," : "";
  console.log(`{
      "vrm": "${vrn()}",
      "start": "${toISODate(licenceStartDate)}",
      "end": "${toISODate(licenceEndDate)}",
      "taxiOrPHV":"${randomVehicleType()}",
      "licensingAuthorityName": "${randomLicensingAuthorityName()}",
      "licensePlateNumber": "${randomLicensePlateNumber()}"
    }${appendComma}`);
}
