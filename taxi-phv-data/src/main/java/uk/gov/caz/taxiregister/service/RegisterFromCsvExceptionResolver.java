...
public class RegisterFromCsvExceptionResolver {
  ...
  private static final long MAX_FILE_SIZE_IN_MEGABYTES =
      TaxiPhvLicenceCsvRepository.MAX_FILE_SIZE_IN_BYTES / 1024 / 1024;

  private static final RegisterResult BUCKET_OR_FILE_NOT_EXISTS = FailureRegisterResult.with(
      ValidationError.s3Error("S3 bucket or file not found or not accessible"));
  private static final RegisterResult ABSENT_UPLOADER_ID = FailureRegisterResult.with(
      ValidationError.s3Error("'uploader-id' not found in file's metadata"));
  private static final RegisterResult MALFORMED_UPLOADER_ID = FailureRegisterResult.with(
  ...
  private static final RegisterResult MAX_FILE_SIZE_EXCEEDED = FailureRegisterResult.with(
      ValidationError.s3Error("Uploaded file size exceeded \"Max size: "
          + MAX_FILE_SIZE_IN_MEGABYTES + "MB\""));
  private static final RegisterResult INTERNAL_ERROR = FailureRegisterResult.with(
  ...
  private static final RegisterResult UNKNOWN = FailureRegisterResult.with(
