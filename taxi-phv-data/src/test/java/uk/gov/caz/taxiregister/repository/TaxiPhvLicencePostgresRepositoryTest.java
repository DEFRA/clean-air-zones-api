...
import uk.gov.caz.taxiregister.repository.TaxiPhvLicencePostgresRepository.DeleteBatchPreparedStatementSetter;
import uk.gov.caz.taxiregister.repository.TaxiPhvLicencePostgresRepository.InsertBatchPreparedStatementSetter;
import uk.gov.caz.taxiregister.repository.TaxiPhvLicencePostgresRepository.LicenceRowMapper;
import uk.gov.caz.taxiregister.repository.TaxiPhvLicencePostgresRepository.UpdateBatchPreparedStatementSetter;
...
class TaxiPhvLicencePostgresRepositoryTest {
  ...
  private static final int ANY_LICENSING_AUTHORITY_ID = 91;
  ...
  private static final TaxiPhvVehicleLicence ANY_TAXI_PHV_VEHICLE_LICENCE = TaxiPhvVehicleLicence
      ...
      .vrm(ANY_VRM)
      .wheelchairAccessible(true).licensePlateNumber("plate")
      .description("TAXI")
      .licenseDates(
          new LicenseDates(
              LocalDate.now(), 
  ...
  private static final Set<TaxiPhvVehicleLicence> ANY_TAXI_PHV_VEHICLE_LICENCES = ImmutableSet
      .of(ANY_TAXI_PHV_VEHICLE_LICENCE.toBuilder().id(1).vrm("1289J").build(),
          ANY_TAXI_PHV_VEHICLE_LICENCE.toBuilder().id(2).vrm("K97LUK").build(),
          ANY_TAXI_PHV_VEHICLE_LICENCE.toBuilder().id(3).vrm("AAA999").build());
  ...
  public void shouldFindAllVehicles() {
    ...
    List<TaxiPhvVehicleLicence> result = taxiPhvLicencePostgresRepository
        .findAll();
  ...
  @Test
  public void shouldFindAllVehiclesForGivenLicensingAuthority() {
    ...
    Set<TaxiPhvVehicleLicence> result = taxiPhvLicencePostgresRepository
        .findByLicensingAuthority(ANY_LICENSING_AUTHORITY_ID);

    assertThat(result).containsExactlyElementsOf(taxiPhvVehicleLicences);
  }

  @Test
  public void shouldSetUploaderIdInPreparedStatementForLicensingAuthority()
      throws SQLException {
    PreparedStatement preparedStatement = mock(PreparedStatement.class);
    mockDataInDatabaseForUploader();

    taxiPhvLicencePostgresRepository.findByLicensingAuthority(ANY_LICENSING_AUTHORITY_ID);
    PreparedStatementSetter value = preparedStmtSetterArgumentCaptor.getValue();
    value.setValues(preparedStatement);

    verify(preparedStatement).setObject(1, ANY_LICENSING_AUTHORITY_ID);
  }

  @Test
  public void shouldSetUploaderIdInPreparedStatementForGivenVrm()
      throws SQLException {
    PreparedStatement preparedStatement = mock(PreparedStatement.class);
  ...
  public void shouldUpdateInBatches() {
    ...
    verify(jdbcTemplate, times(2)).batchUpdate(
        eq(TaxiPhvLicencePostgresRepository.UPDATE_SQL),
  ...
  public void shouldInsertInBatches() {
    ...
    verify(jdbcTemplate, times(2)).batchUpdate(
        eq(TaxiPhvLicencePostgresRepository.INSERT_SQL),
        any(BatchPreparedStatementSetter.class));
  }

  @Test
  public void shouldDeleteInBatches() {
    Set<Integer> vehicles = ANY_TAXI_PHV_VEHICLE_LICENCES.stream()
        .map(TaxiPhvVehicleLicence::getId).collect(Collectors.toSet());
  ...
  class RowMapper {
    ...
    @Test
    public void shouldMapResultSetToVehicleWithWheelchairAccessToTrueWhenFlagSetToY()
        throws SQLException {
      ResultSet resultSet = mockResultSetForWheelchairAccessFlagSetTo("y");

      TaxiPhvVehicleLicence taxiPhvVehicleLicence = rowMapper.mapRow(resultSet,
          0);
    ...
    @Test
    public void shouldMapResultSetToVehicleWithWheelchairAccessToFalseWhenFlagSetToN()
        throws SQLException {
    ...
    @Test
    public void shouldMapResultSetToVehicleWithWheelchairAccessToNullWhenFlagSetToNull()
        throws SQLException {
    ...
    @Test
    public void shouldMapResultSetToVehicleWithDescriptionSetToPhv()
        throws SQLException {
      ResultSet resultSet = mockResultSetForDescriptionSetTo("PHV");

      TaxiPhvVehicleLicence taxiPhvVehicleLicence = rowMapper.mapRow(resultSet,
          0);

      assertThat(taxiPhvVehicleLicence.getDescription()).isSameAs("PHV");
    }

    @Test
    public void shouldMapResultSetToVehicleWithAnyValidValues()
        throws SQLException {
    ...
    private ResultSet mockResultSetForWheelchairAccessFlagSetTo(String n)
        throws SQLException {
      return mockResultSet(n, "TAXI");
    }

    private ResultSet mockResultSetForDescriptionSetTo(String description)
        throws SQLException {
      return mockResultSet("y", description);
    }

    private ResultSet mockResultSet(String wheelchairAccessFlagValue,
  ...
  class InsertPreparedStatementSetter {
    ...
    public void shouldSetPreparedStatementAttributes() throws SQLException {
      ...
      InsertBatchPreparedStatementSetter setter = createSetter(
      ...
      verify(preparedStatement).setString(2,
          taxiPhvVehicleLicence.getDescription());
      verify(preparedStatement).setObject(3,
          taxiPhvVehicleLicence.getLicenseDates().getStart());
      verify(preparedStatement).setObject(4,
          taxiPhvVehicleLicence.getLicenseDates().getEnd());
      verify(preparedStatement).setInt(5,
          taxiPhvVehicleLicence.getLicensingAuthority().getId());
      verify(preparedStatement).setString(6,
          taxiPhvVehicleLicence.getLicensePlateNumber());
      verify(preparedStatement).setString(7,
          Optional.ofNullable(taxiPhvVehicleLicence.getWheelchairAccessible())
              .map(wheelchair -> wheelchair ? "y" : "n").orElse(null));
      verify(preparedStatement).setObject(8,
          taxiPhvVehicleLicence.getUploaderId());
    }

    @Test
    public void shouldSetPreparedStatementAttributeWhenWheelchairFlagIsFalse()
        throws SQLException {
      TaxiPhvVehicleLicence taxiPhvVehicleLicence = ANY_TAXI_PHV_VEHICLE_LICENCE
          .toBuilder()
    ...
    public void shouldReturnBatchSize() {
      List<TaxiPhvVehicleLicence> input = Collections
          .singletonList(ANY_TAXI_PHV_VEHICLE_LICENCE);
  ...
  class UpdatePreparedStatementSetter {
    ...
    public void shouldSetPreparedStatementAttributes() throws SQLException {
      ...
      verify(preparedStatement).setString(6,
          taxiPhvVehicleLicence.getLicensePlateNumber());
      verify(preparedStatement).setString(7,
          taxiPhvVehicleLicence.getWheelchairAccessible() ? "y" : "n");
      verify(preparedStatement).setObject(8,
          taxiPhvVehicleLicence.getUploaderId());
  ...
  class DeletePreparedStatementSetter {
    ...
    public void shouldSetPreparedStatementAttributes() throws SQLException {
      ...
      PreparedStatement preparedStatement = mock(PreparedStatement.class);
      DeleteBatchPreparedStatementSetter setter = new DeleteBatchPreparedStatementSetter(
          batch);
  ...
  private List<TaxiPhvVehicleLicence> mockDataInDatabaseForUploader() {
    List<TaxiPhvVehicleLicence> taxiPhvVehicleLicences = Collections
        .singletonList(ANY_TAXI_PHV_VEHICLE_LICENCE);
    when(jdbcTemplate.query(
        eq(TaxiPhvLicencePostgresRepository.SELECT_BY_LICENSING_AUTHORITY_ID_SQL),
        preparedStmtSetterArgumentCaptor.capture(),
        any(LicenceRowMapper.class))).thenReturn(taxiPhvVehicleLicences);
  ...
  private List<TaxiPhvVehicleLicence> mockDataInDatabaseForFindAll() {
    List<TaxiPhvVehicleLicence> taxiPhvVehicleLicences = Collections
        .singletonList(ANY_TAXI_PHV_VEHICLE_LICENCE);
    when(jdbcTemplate.query(eq(TaxiPhvLicencePostgresRepository.SELECT_ALL_SQL),
        any(LicenceRowMapper.class))).thenReturn(taxiPhvVehicleLicences);
    return taxiPhvVehicleLicences;
  }
}