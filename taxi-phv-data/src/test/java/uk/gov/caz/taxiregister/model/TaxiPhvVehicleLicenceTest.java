...
class TaxiPhvVehicleLicenceTest {
  ...
  class isActive {
    ...
    @ParameterizedTest
    @MethodSource("uk.gov.caz.taxiregister.model.TaxiPhvVehicleLicenceTest#activeLicenceDatesProvider")
    public void shouldReturnTrueForActiveLicence(LocalDate start,
        LocalDate end) {
      // given
    ...
    @ParameterizedTest
    @MethodSource("uk.gov.caz.taxiregister.model.TaxiPhvVehicleLicenceTest#inactiveLicenceDatesProvider")
    public void shouldReturnFalseForInactiveLicence(LocalDate start,
        LocalDate end) {
      // given
      TaxiPhvVehicleLicence licence = licenceWith(start, end);
  ...
  private TaxiPhvVehicleLicence licenceWith(LocalDate startDate,
      LocalDate endDate) {
    return TaxiPhvVehicleLicence.builder().id(1).uploaderId(UUID.randomUUID())
        .vrm("AAA999").wheelchairAccessible(true).licensePlateNumber("old")
        .description("TAXI")
        .licenseDates(new LicenseDates(startDate, endDate))
        .licensingAuthority(new LicensingAuthority(99, "la")).build();
  }
}