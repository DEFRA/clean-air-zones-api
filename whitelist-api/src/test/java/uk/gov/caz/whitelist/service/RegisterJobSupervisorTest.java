...
class RegisterJobSupervisorTest {
  ...
  class activeJobs {
    ...
    public void isRunningOrStartingJob() {
      // when
      registerJobSupervisor.hasActiveJobsFor(CsvContentType.WHITELIST_LIST);
    ...
    public void testHasActiveJobsIsFalseWhenNoJobsArePresent() {
      // given
      given(
          mockedRegisterJobRepository.countActiveJobsByContentType(CsvContentType.WHITELIST_LIST))
          .willReturn(null);

      // when
      boolean areThereActiveJobs = registerJobSupervisor
          .hasActiveJobsFor(CsvContentType.WHITELIST_LIST);
  ...
  private StartParams prepareStartParams(AtomicBoolean capturedJobStarted,
    ...
    return StartParams.builder()
        .registerJobTrigger(S3_WHITELIST_REGISTER_JOB_TRIGGER)
  ...
  private void prepareMocksForNameGenerationAndRegisterJobInsertion() {
    given(mockedRegisterJobNameGenerator.generate(CSV_FILE, S3_WHITELIST_REGISTER_JOB_TRIGGER))
