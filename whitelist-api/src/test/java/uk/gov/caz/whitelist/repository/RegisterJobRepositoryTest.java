...
import static uk.gov.caz.whitelist.repository.RegisterJobRepository.COL_CORRELATION_ID;
import static uk.gov.caz.whitelist.repository.RegisterJobRepository.COL_ERRORS;
import static uk.gov.caz.whitelist.repository.RegisterJobRepository.COL_JOB_NAME;
import static uk.gov.caz.whitelist.repository.RegisterJobRepository.COL_REGISTER_JOB_ID;
import static uk.gov.caz.whitelist.repository.RegisterJobRepository.COL_STATUS;
import static uk.gov.caz.whitelist.repository.RegisterJobRepository.COL_TRIGGER;
...
import uk.gov.caz.whitelist.repository.RegisterJobRepository.RegisterJobRowMapper;
  ...
  class RowMapper {
    ...
    private ResultSet mockResultSetWithErrorsEqualTo(String errors) throws SQLException {
      ...
      when(resultSet.getString(anyString())).thenAnswer(answer -> {
        ...
        switch (argument) {
          case COL_TRIGGER:
            return S3_WHITELIST_REGISTER_JOB_TRIGGER.name();
          case COL_JOB_NAME:
