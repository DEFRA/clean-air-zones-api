...
public class RegisterFromCsvExceptionResolver {
  ...
  private static final RegisterResult ABSENT_UPLOADER_ID = RegisterResult.failure(
      ValidationError.s3Error("'" + AccountVehicleDtoCsvRepository.UPLOADER_ID_METADATA_KEY + "' "
          + "not found in file's metadata."));
  ...
  private static final RegisterResult MAX_FILE_SIZE_EXCEEDED = RegisterResult.failure(
      ValidationError.s3Error("Uploaded file is too large. Maximum allowed: "
          + AccountVehicleDtoCsvRepository.MAX_FILE_SIZE_IN_BYTES + " bytes."));
  private static final RegisterResult EMPTY_FILE_UPLOADED = RegisterResult.failure(
      ValidationError.fileError("Your CSV did not contain any number plates. "
  ...
  private static final Map<Class<? extends Exception>, RegisterResult> EXCEPTION_TO_RESULT
      ...
      InvalidRequestPayloadException.class, EMPTY_FILE_UPLOADED
  );
