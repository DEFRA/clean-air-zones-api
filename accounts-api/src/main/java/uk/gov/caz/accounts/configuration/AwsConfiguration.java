...
import uk.gov.caz.accounts.repository.CognitoClientProvider;
...
import uk.gov.caz.accounts.repository.StandardCognitoClientProvider;
import uk.gov.caz.accounts.repository.StandardCognitoIdentityProviderClient;
import uk.gov.caz.awslambda.AwsHelpers;

/**
 * Spring Configuration class for {@link CognitoIdentityProviderClient}.
 */
@Configuration
@Slf4j
public class AwsConfiguration {
  ...
  /**
   * Creates and initializes {@link CognitoClientProvider}.
   *
   * @return A configured CognitoClientProvider.
   */
  @Bean
  @Profile("!integration-tests")
  public CognitoClientProvider cognitoClientProvider(
      @Value("${cognito.sdk.secret:}") String cognitoSecretsSource,
      AWSSecretsManager awsSecretsManager,
      ObjectMapper objectMapper) {
    if (StringUtils.isBlank(cognitoSecretsSource)) {
      log.info("Using default StandardCognitoClientProvider");
      return new StandardCognitoClientProvider();
    }
    log.info("Using SecretServiceBasedCognitoClientProvider");
    return new SecretServiceBasedCognitoClientProvider(cognitoSecretsSource, awsSecretsManager,
        objectMapper);
  }

  /**
   * Creates and initialises {@link uk.gov.caz.accounts.repository.CognitoIdentityProviderClient}
   * based on the presence of {@code cognito.sdk.secret} value.
   */
  @Bean
  public uk.gov.caz.accounts.repository.CognitoIdentityProviderClient cognitoIdentityProviderClient(
      @Value("${cognito.sdk.secret:}") String cognitoSecretsSource,
      CognitoClientProvider cognitoClientProvider) {
    if (StringUtils.isBlank(cognitoSecretsSource)) {
      log.info("Using default StandardCognitoIdentityProviderClient");
      return new StandardCognitoIdentityProviderClient(cognitoClientProvider);
    }
    log.info("Using CredentialsRotatingCognitoIdentityProviderClient");
    return new CredentialsRotatingCognitoIdentityProviderClient(cognitoClientProvider);
  }
  ...
  public AmazonSQS sqsLocalstackClient(@Value("${aws.sqs.endpoint:}") String sqsEndpoint) {
    ...
    return AmazonSQSClientBuilder.standard().withCredentials(dummyCredentialsProvider())
        .withEndpointConfiguration(
            new EndpointConfiguration(sqsEndpoint, Region.EU_WEST_2.toString())).build();
  }
  ...
  /**
   * Returns an instance of {@link AWSSecretsManager} which is used to to retrieve secrets from AWS
   * secrets manager.
   *
  ...
  public AWSSecretsManager secretsManagerLocalstackClient(
    ...
    return AWSSecretsManagerClientBuilder.standard().withCredentials(dummyCredentialsProvider())
        .withEndpointConfiguration(
            new EndpointConfiguration(secretsManagerEndpoint, Region.EU_WEST_2.toString())
        )
        .build();
  }
  ...
  public AWSSecretsManager awsSecretsManagerClientBuilder() {
    return AWSSecretsManagerClientBuilder.standard().withRegion(Region.EU_WEST_2.toString())
        .build();
  }
}
