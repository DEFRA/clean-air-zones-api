...
@AllArgsConstructor
@Slf4j
public class AccountVehicleDtoCsvRepository {

  public static final String UPLOADER_ID_METADATA_KEY = "account-user-id";
  public static final long MAX_FILE_SIZE_IN_BYTES = 100L * 1024 * 1024; // 100 MB

  private final S3Client s3Client;
  private final CsvAccountVehicleObjectMapper csvAccountVehicleObjectMapper;
  ...
  public CsvFindResult findAll(String bucket, String filename) {
    Preconditions.checkArgument(!Strings.isNullOrEmpty(bucket),
        "Bucket %s cannot be null or empty");
    Preconditions.checkArgument(!Strings.isNullOrEmpty(filename),
        "Filename %s cannot be null or empty");
    ...
    try (InputStream inputStream = getS3FileInputStream(bucket, filename)) {
      CsvParseResult<AccountVehicleDto> result = csvAccountVehicleObjectMapper.read(inputStream);
      return new CsvFindResult(uploaderId, result.getObjects(),
  ...
  private UUID getUploaderId(HeadObjectResponse fileMetadata) {
    try {
      UUID accountUserId = UUID.fromString(fileMetadata.metadata().get(UPLOADER_ID_METADATA_KEY));
