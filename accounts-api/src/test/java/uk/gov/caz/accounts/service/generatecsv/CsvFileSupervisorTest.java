...
class CsvFileSupervisorTest {
  ...
  public void shouldThrowS3ExceptionsDuringUploadingCsv() throws IOException {
    ...
    Throwable throwable = catchThrowable(
        () -> csvFileSupervisor.uploadCsvFileAndGetPresignedUrl(UUID.randomUUID()));
  ...
  private void mockIOException() throws IOException {
    given(csvWriter.createWriterWithCsvContent(any())).willThrow(IOException.class);
  }
  ...
  private void mockCreateWriterWithCsvContent() throws IOException {
    given(csvWriter.createWriterWithCsvContent(any())).willReturn(new StringWriter());
  }
