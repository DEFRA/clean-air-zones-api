...
@IntegrationTest
public class CsvFileSupervisorTestIT extends ExternalCallsIT {

  private static final String BUCKET_NAME = "fleet-csv-export-bucket";
  ...
  public void shouldUploadCsvFileToS3() throws IOException {
    ...
    assertThat(getKey()).contains("fleet-7dfcd5c6c67a").contains(".csv");
    assertThat(getUploadedCsvFile()).contains(readExpectedCsv());
    assertThat(url).isNotNull();
  }

  @Test
  public void shouldUploadCsvFileWithOnlyHeaderRow() throws IOException {
    // given
    UUID accountId = UUID.fromString("1f30838f-69ee-4486-95b4-7dfcd5c6c67b");
    ...
    // then
    assertThat(getS3Contents()).hasSize(1);
    assertThat(getKey()).contains("fleet-7dfcd5c6c67b").contains(".csv");
    assertThat(getUploadedCsvFile()).contains(readExpectedCsvOnlyWithHeaderRow());
    assertThat(url).isNotNull();
  }
  ...
  private static String readExpectedCsv() {
    return getCsvFile("classpath:data/csv/export/expected-vehicles-with-charges.csv");
  }

  private static String readExpectedCsvOnlyWithHeaderRow() {
    return getCsvFile("classpath:data/csv/export/header-row.csv");
  }
